...
gulp.task('test', function() {
  return gulp.src(['**/**_test.js', '!node_modules/**/*'], {
    read: false
  })
    .pipe(mocha({
    reporter: 'spec'
...
gulp.task('release', ['test'], function() {
  ...
  // this is the task to automat most of the release stuff...
  // because it is lame and boring
  console.log('\n\nPreparing for a ' +
    chalk.bgGreen.bold(options.type) +
    ' release...\n\n'
  );


  cp.exec('git log `git describe --tags --abbrev=0`..HEAD ' +
          '--pretty=format:" - %s"',
    ...
    fs.writeFile('./history.md', '### - ' + newVersion +
      ' *' + new Date().toLocaleString() + '*\n\n' + stdout +
      '\n\n\n' + history);

    cp.exec('git log --all --format="%aN <%aE>" | sort -u',
      function(err, stdout, stderr) {
